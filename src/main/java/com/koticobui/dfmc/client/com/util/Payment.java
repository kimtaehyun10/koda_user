...
public class Payment {
	...
	private final boolean KeepConnection = false;
	...
	public String genTableKey() throws Exception {
		...
		try {
			...
			final String today = simpleDateFormat.format(currentTime);
	...
	public Map savePay(HashMap map) {
		...
		try {
			...
			//OCR번호(고객조회번호) 조회
			//수납키로 OCR번호 조회
			final String ocrNo = selectOcrNo(map);
			System.out.println("ocrNo:"+ocrNo);

			//결제키 생성
			final String payKey = genTableKey();
			...
			if(rlt_code > 0) {
				...
				final Map<String, String> map1 = selectPayCnt(map);
