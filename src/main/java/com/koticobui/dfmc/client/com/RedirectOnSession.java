...
public class RedirectOnSession implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if (session.getAttribute("isLocallyAuthenticated") != null) {
			final String auth = session.getAttribute("isLocallyAuthenticated").toString();
			...
			if (auth.equals("true") && uri.equals("/com/selectSession.json") == false) {
				...
				final ApplicationContext act = WebApplicationContextUtils.getRequiredWebApplicationContext(application);
				final SessionService sessionService = (SessionService) act.getBean("sessionService");
				try {
