 ...
 *******************************************************************/
	...
	function checkForm(f) {
		...
		$("#"+f+" input, #"+f+" select, #"+f+" textarea").each(function(){ 
			...
			if(fTyp == 'TEXTAREA' && $(this).hasClass('ckeditor') && !!fMsg){
				var val = eval('CKEDITOR.instances.' + $(this).attr('name') + '.getData()');
				if(!val){
					alert(fMsg + " 입력해 주세요");
					rtnV = false;
					return false;
			...
			if (chkBool && fMsg != undefined && (fTyp == "TEXT" || fTyp == "HIDDEN" || (fTyp == "TEXTAREA" && !$(this).hasClass('ckeditor')) || fTyp == "PASSWORD") && fVal.replace(/ /gi,"") == "") {				
				alert(fMsg + " 입력해 주세요");
