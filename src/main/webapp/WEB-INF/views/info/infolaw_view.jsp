...
<body>
	...
	<input type="hidden" name="searchValue" id="searchValue" value="${libSearch.searchValue }"/>
	<input type="hidden" name="currentPage" id="currentPage" value="${libSearch.currentPage }"/>
	<input type="hidden" name="idx" id="idx" value="${libSearch.brdContNo }"/>
	...
	<div class="wrap">
                ...
                <div class="memorial__detail-content">
                    <header>
                        <h3 class="title">
						<c:out value="${lib.brdTitle }"/>
                        </h3>
                        <div class="info"> 
                            <div class="col">
                               	 등록일 <strong>
							<fmt:parseDate var="regdate" value="${lib.regDm }" pattern="yyyy-MM-dd"/>
							<fmt:formatDate value="${regdate }" pattern="yyyy-MM-dd"/></strong>
                            </div>                            
                            <div class="col">
                                	조회수 <strong><c:out value="${lib.brdReadNum }"/></strong>
                            </div>
                        </div>
                        <c:if test="${not empty lib.brdFileOrgNm }">
                        <div class="attachFile flex items-center">
                            <span class="inline-block">첨부파일</span>
                            <a href="#" onclick="goDown('${lib.brdFileOrgNm}','/${lib.brdFileNm}');"><c:out value="${lib.brdFileOrgNm }"/></a>
                        </div>
						</c:if>
                    </header>
