	...
	jQuery(document).ready(function() {
		...
		function addComment(){
			if(checkForm('frm_url')){
				...
				$.ajax({
		            url: "/donate/story_com_insert.json",
		            type: "POST",
		            ...
		            success: function (data, textStatus, jqXHR) {
		            	if (data.result == "OK"){
							...
							$('#reloadYn').val('Y');
							$('#frm_url').attr({action: '/donate/story_view.c', method: 'post'}).submit();
		            	}else{
			...
			$.ajax({
		           ...
		           success: function (data, textStatus, jqXHR) {
		           	if (data.result == "comDelSuccess"){
						...
						$('#pwdYn').val('Y');
						$('#frm_url').attr({action: '/donate/story_form.c', method: 'post'}).submit();
	...
	<div class="wrap">
                ...
                <div class="memorial__detail">
                        ...
                        <div class="info">
							...
							<c:choose>
                                <c:when test="${story.storyAnonymity eq 'Y' }">
									<c:out value="${fn:substring(story.storyWriter, 0, fn:length(story.storyWriter) - 2) }" />**
								</c:when>
								<c:otherwise>
                            ...
                            <div class="col">
                                조회 <span><c:out value="${story.storyHits }"/></span>
                            </div>
                        </div>
                    </header>
                    <div class="memorial__detail-body">
						...
						${cfn:nl2br(story.storyContents) }</p>
                        </div>
					...
					<a href="javascript:goStoryList();" class="">목록</a>
                    </div>
                </div>
