	...
	<script type="text/javascript">
		...
		function addComment(){
			if(checkForm('frm_url')){
				...
				$.ajax({
		            url: "/donate/letter_com_insert.json",
		            type: "POST",
		            ...
		            success: function (data, textStatus, jqXHR) {
		            	if (data.result == "OK"){
							...
							$('#reloadYn').val('Y');
							$('#frm_url').attr({action: '/donate/letter_view.c', method: 'post'}).submit();
		            	}else{
		...
		function deleteCommentView(idx){
			...
			$.ajax({
		           ...
		           success: function (data, textStatus, jqXHR) {
		           	if (data.result == "comDelSuccess"){
						...
						$('#pwdYn').val('Y');
						$('#frm_url').attr({action: '/donate/letter_form.c', method: 'post'}).submit();
...
<body>
	...
	<input type="hidden" name="comCurrentPage" id="comCurrentPage" value="${comDonateSearch.currentPage }"/>
                ...
                <div class="memorial__detail">
                    <header>
                        ...
                        <div class="info">
                            <div class="col">
								...
								<c:choose>
									<c:when test="${letter.donateAnonymity eq 'Y' }">
										<c:set var="donateNm" value="${fn:substring(letter.donateName, 0, 1) }*${fn:substring(letter.donateName, 2, fn:length(donate.donateName) - 1) }"/>
									</c:when>
									<c:otherwise>
										<c:set var="donateNm" value="${letter.donateName }"/>
									</c:otherwise>
								</c:choose>
								<span><a href="javascript:goMemberView(${letter.donateIdx });"><c:out value="${donateNm }"/></a></span>
							</c:if>
                            </div>
                            <div class="col">
                                	추모자
                            ...
                            <div class="col">
                                	조회 <span><c:out value="${letter.storyHits }"/></span>
                            </div>
                        </div>
                    </header>
                    <div class="memorial__detail-body">
                        ...
                        <div class="detail" >
                            <p style="white-space: pre-line;line-height: 2em;">${cfn:nl2br(letter.storyContents)}</p>
                        </div>
                    ...
                    <div class="memorial__detail-foot">
                        ...
                        <a href="javascript:goLetterList();" class="">목록</a>
                    </div>
                </div>
                ...
                <div class="memorial__comment">
                    <div class="memorial__comment-author">
                        ...
                        <div class="col">
                            <strong>비밀번호</strong>
                            <div class="input">
                            	<input type="password" id="upw" name="upw" name="upw" msg="비밀번호를" onkeypress="if(event.keyCode == 13) return false;">
                            </div>
                        </div>
                    </div>
                    <div class="memorial__comment-form">
                        <div class="textarea">
                            <textarea name="comContent" id="comContent" placeholder="기증자에 대한 추모 분위기를 해치거나, 비방의 글 등이 게시가 될 경우 관리자에 의해 삭제 될 수 있습니다. "></textarea>
                        </div>
                        <button type="button" onclick="javascript:addComment();" title="댓글등록">댓글등록</button>
                    </div>
							...
							<c:when test="${not empty comList and comDonateSearch.totalCount > 0 }">
								...
								<div class="memorial__comment-title">
	                                <div class="info">
	                                    ...
	                                    <a href="javascript:deleteCommentView(${com.comIdx })">삭제</a>
	                                </div>
	                                <p>
