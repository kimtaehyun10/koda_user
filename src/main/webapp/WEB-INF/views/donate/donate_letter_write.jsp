...
<head>
	...
	<script type="text/javascript">
		...
		function donateSelPop(){
			$('#esignup-1').css('display', 'flex');
			//window.open('/remembrance/donate_pop.do', 'donateSel', 'width=700, height=750, scrollbars=yes, resizable=no');
		}
					...
					$.ajax({
			            ...
			            success: function (data, textStatus, jqXHR) {
			            	if (data.result == "insertSuccess"){
								...
								$('#frm_url').attr({action: '/donate/letter.c', method: 'post'}).submit();
			            	}else{
...
<body>
	...
	<input type="hidden" name="donateIdx" id="donateIdx" value="${letter.donateIdx }" />
                ...
                <div class="memorial__message">
                        ...
                        <table>
                                ...
                                <td>
                                    <div class="flex has-ck">
                                        <div class="input w350">
                                        	<input type="text" id="writer" name="writer" value="<c:out escapeXml="true" value="${letter.storyWriter }"/>" msg="추모자를"/> 
                                        </div>

                                        <label for="anonymity" class="checkbox">
                                            <input type="checkbox" id="anonymity" name="anonymity" hidden value="Y" <c:if test="${letter.storyAnonymity eq 'Y' }">checked="checked"</c:if>>
                                            <span></span>
                                            <p>익명으로 작성하기</p>
                                        </label>
                                    </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="flex">
                                        <div class="input w350">
											<input type="password" class="input" id="pwd" name="pwd" msg="비밀번호를"/>
                                        </div>
                                    </div>
                                </td>
                            </tr>
                                        ...
                                        <div class="input w350">
                                        	<c:choose>
												<c:when test="${letter.donateAnonymity eq 'Y' }">
													<c:set var="donateNm" value="${fn:substring(letter.donateName, 0, 1) }*${fn:substring(letter.donateName, 2, fn:length(letter.donateName) - 1) }"/>
												</c:when>
												<c:otherwise>
													<c:set var="donateNm" value="${letter.donateName }"/>
												</c:otherwise>
											</c:choose>
											<input type="text" name="donateName" id="donateName" value="<c:out escapeXml="true" value="${donateNm }"/>" readonly onClick="donateSelPop(); return false;">
                                        </div>
                                        <button type="button" class="submit" onClick="donateSelPop(); return false;" title="검색하기">검색</button>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    <div class="flex">
                                        <div class="input">
                                            <input type="text" id="title" name="title" value="<c:out escapeXml="true" value="${letter.storyTitle }"/>" msg="제목을"/>
                                        </div>
                                    </div>
                                </td>
                            </tr>
                            <tr>
                                <td colspan="2" style="padding: 10px 0;">
                                    <textarea class="ckeditor" id="contents" name="contents" rows="5" msg="내용을"><c:out escapeXml="true" value="${letter.storyContents }"/></textarea>
                                </td>
                            </tr>
                            </tbody>
                        </table>
                    ...
                    <div class="form-actions justify-center">
                    	<c:if test="${empty donateSearch.idx or donateSearch.idx eq 0 }">
                        	<button type="button" onclick="javascript:addLetter();" class="btn submit">등록</button>
						</c:if>
						<c:if test="${donateSearch.idx > 0 }">
                        	<button type="button" onclick="javascript:modifyLetter();" class="btn submit">수정</button>
                        	<a href="javascript:deleteLetter();" class="btn">삭제</a>
						</c:if>
            ...
            <div class="modal modal--eisngup" style="display: none;" id="esignup-1">
				<div class="modal__content">
					...
					<div class="modal__body">
								...
								<table>
									...
									<tbody>
										<tr>
											...
											<td colspan="2">
												<div class="input">
													<input type="text" id="donateName" name="donateName" title="기증자명">
												</div>
											</td>
										</tr>
										<tr>
							...
							<button type="button" class="submit" onclick="javascript:deleteComment()">확인</button>
					</div>
				</div>
			</div>	
			</div>
